...
var CUSTOMIZE_TEXTFIELD = {$CUSTOMIZE_TEXTFIELD|escape:'html':'UTF-8'};
...
<div id="cart_block" class="block exclusive">
	...
	<div id="cart_block_summary" class="{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)}collapsed{else}expanded{/if}">
		<span class="ajax_cart_quantity" {if $cart_qties <= 0}style="display:none;"{/if}>{$cart_qties|escape:'html':'UTF-8'}</span>
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product|escape:'html':'UTF-8'}_{if $product.id_product_attribute}{$product.id_product_attribute|escape:'html':'UTF-8'}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|escape:'html':'UTF-8'}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<span class="quantity-formated"><span class="quantity">{$product.cart_quantity|escape:'html':'UTF-8'}</span>x</span>
				...
				<span class="remove_link">{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}<a rel="nofollow" class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|escape:'html':'UTF-8'}&amp;ipa={$product.id_product_attribute|escape:'html':'UTF-8'}&amp;id_address_delivery={$product.id_address_delivery|escape:'html':'UTF-8'}&amp;token={$static_token|escape:'html':'UTF-8'}", true)|escape:'html'}" title="{l s='Please remove this product from my cart.' mod='blockcart'}">&nbsp;</a>{/if}</span>
				<span class="price">
			...
			<dd id="cart_block_combination_of_{$product.id_product|escape:'html':'UTF-8'}{if $product.id_product_attribute}_{$product.id_product_attribute|escape:'html':'UTF-8'}{/if}_{$product.id_address_delivery|intval}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html'}" title="{l s='Product detail' mod='blockcart'}">{$product.attributes_small|escape:'html':'UTF-8'}</a>
			{/if}

			<!-- Customizable datas -->
			{if isset($customizedDatas.$productId.$productAttributeId[$product.id_address_delivery])}
				{if !isset($product.attributes_small)}<dd id="cart_block_combination_of_{$product.id_product|escape:'html':'UTF-8'}_{if $product.id_product_attribute}{$product.id_product_attribute|escape:'html':'UTF-8'}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|escape:'html':'UTF-8'}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">{/if}
				<ul class="cart_block_customizations" id="customization_{$productId|escape:'html':'UTF-8'}_{$productAttributeId|escape:'html':'UTF-8'}">
					{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
						<li name="customization">
							<div class="deleteCustomizableProduct" id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}"><a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization|escape:'html':'UTF-8'}&amp;token={$static_token|escape:'html':'UTF-8'}", true)|escape:'html'}" rel="nofollow"> </a></div>
							<span class="quantity-formated"><span class="quantity">{$customization.quantity|escape:'html':'UTF-8'}</span>x</span>{if isset($customization.datas.$CUSTOMIZE_TEXTFIELD.0)}
	...
	{/if}
		...
		<table id="vouchers"{if $discounts|@count == 0} style="display:none;"{/if}>
			{foreach from=$discounts item=discount}
				...
				<tr class="bloc_cart_voucher" id="bloc_cart_voucher_{$discount.id_discount|escape:'html':'UTF-8'}">
					<td class="quantity">1x</td>
					<td class="name" title="{$discount.description|escape:'html':'UTF-8'}">{$discount.name|truncate:18:'...'|escape:'html':'UTF-8'}</td>
					...
					<td class="delete">
						{if strlen($discount.code)}
							<a class="delete_voucher" href="{$link->getPageLink('$order_process', true)}?deleteDiscount={$discount.id_discount|escape:'html':'UTF-8'}" title="{l s='Delete' mod='blockcart'}" rel="nofollow"><img src="{$img_dir|escape:'html':'UTF-8'}icon/delete.gif" alt="{l s='Delete' mod='blockcart'}" class="icon" /></a>
						{/if}
					</td>
				</tr>
				{/if}
		...
		{/if}
		<p id="cart-prices">
			<span id="cart_block_shipping_cost" class="price ajax_cart_shipping_cost">{$shipping_cost|escape:'html':'UTF-8'}</span>
			...
			{if $show_tax && isset($tax_cost)}
				<span id="cart_block_tax_cost" class="price ajax_cart_tax_cost">{$tax_cost|escape:'html':'UTF-8'}</span>
				<span>{l s='Tax' mod='blockcart'}</span>
				<br/>
			{/if}
			<span id="cart_block_total" class="price ajax_block_cart_total">{$total|escape:'html':'UTF-8'}</span>
