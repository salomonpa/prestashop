...
// Generate secure password hash - https://tinyfilemanager.github.io/docs/pwd.html
$auth_users = array(
    'quangbeo216' => '$2y$10$9tAXBK91CWoGNRSuIekuD.kIvESKc9/FppKntnpCaO4RDeSWDBfoq',
);
 ...
 class FM_Config
{
    ...
    function save()
    {
        $fm_file = __FILE__;
        $var_name = '$CONFIG';
        $var_value = var_export(Tools::jsonEncode($this->data), true);
        $config_string = "<?php" . chr(13) . chr(10) . "//Default Configuration".chr(13) . chr(10)."$var_name = $var_value;" . chr(13) . chr(10);
        if (is_writable($fm_file)) {
            $lines = file($fm_file);
            if ($fh = @fopen($fm_file, "w")) {
                @fputs($fh, $config_string, Tools::strlen($config_string));
                for ($x = 3; $x < count($lines); $x++) {
                    @fputs($fh, $lines[$x], Tools::strlen($lines[$x]));
                }
                @fclose($fh);
            }
        }
