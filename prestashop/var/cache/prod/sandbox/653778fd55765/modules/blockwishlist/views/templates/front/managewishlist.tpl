...
{if $products}
	...
	<div class="wlp_bought">
		...
		{foreach from=$products item=product name=i}
			<li id="wlp_{$product.id_product|escape:'html':'UTF-8'}_{$product.id_product_attribute|escape:'html':'UTF-8'}" class="clearfix address {if $smarty.foreach.i.index % 2}alternate_{/if}item">
				<a href="javascript:;" class="lnkdel" onclick="WishlistProductManage('wlp_bought', 'delete', '{$id_wishlist|escape:'html':'UTF-8'}', '{$product.id_product|escape:'html':'UTF-8'}', '{$product.id_product_attribute|escape:'html':'UTF-8'}', $('#quantity_{$product.id_product|escape:'html':'UTF-8'}_{$product.id_product_attribute|escape:'html':'UTF-8'}').val(), $('#priority_{$product.id_product|escape:'html':'UTF-8'}_{$product.id_product_attribute|escape:'html':'UTF-8'}').val());" title="{l s='Delete' mod='blockwishlist'}">&raquo; {l s='Delete' mod='blockwishlist'}</a>
				<div class="clearfix">
					...
					<div class="product_infos">
						...
						{/if}
							<br />{l s='Quantity' mod='blockwishlist'}:<input type="text" id="quantity_{$product.id_product|escape:'html':'UTF-8'}_{$product.id_product_attribute|escape:'html':'UTF-8'}" value="{$product.quantity|intval}" size="3"  />
							<br /><br />
							{l s='Priority' mod='blockwishlist'}:
							<select id="priority_{$product.id_product|escape:'html':'UTF-8'}_{$product.id_product_attribute|escape:'html':'UTF-8'}">
							...
							{if $wishlists|count > 1}
                                ...
                                {foreach name=wl from=$wishlists item=wishlist}
                                    ...
                                    {if $id_wishlist != {$wishlist.id_wishlist|escape:'html':'UTF-8'}}
	                                        <option title="{$wishlist.name|escape:'html':'UTF-8'}" value="{$wishlist.id_wishlist|escape:'html':'UTF-8'}" data-id-product="{$product.id_product|escape:'html':'UTF-8'}" data-id-product-attribute="{$product.id_product_attribute|escape:'html':'UTF-8'}" data-quantity="{$product.quantity|intval}" data-priority="{$product.priority|escape:'html':'UTF-8'}" data-id-old-wishlist="{$id_wishlist|escape:'html':'UTF-8'}" data-id-new-wishlist="{$wishlist.id_wishlist|escape:'html':'UTF-8'}">
	...
	<form method="post" class="wl_send std" onsubmit="return (false);" style="display: none;">
		...
		<fieldset>
			...
			{/section}
			<p class="submit">
				<input class="button" type="submit" value="{l s='Send' mod='blockwishlist'}" name="submitWishlist" onclick="WishlistSend('wl_send', '{$id_wishlist|escape:'html':'UTF-8'}', 'email');" />
			</p>
			<p class="required">
