...
{assign var='productAttributeId' value=$product.id_product_attribute}
	{ldelim}
		"id":            {$product.id_product|escape:'html':'UTF-8'},
		...
		"price_float":   "{$product.total|escape:'html':'UTF-8'}",
		"idCombination": {if isset($product.attributes_small)}{$productAttributeId|escape:'html':'UTF-8'}{else}0{/if},
		"idAddressDelivery": {if isset($product.id_address_delivery)}{$product.id_address_delivery|escape:'html':'UTF-8'}{else}0{/if},
...
{* This empty line was made in purpose (product addition debug), please leave it here *}
			"customizationId":	{$id_customization|escape:'html':'UTF-8'},
			"quantity":			"{$customization.quantity|escape:'html':'UTF-8'}",
			"datas": [
				{foreach from=$customization.datas key='type' item='datas' name='customization'}
				{ldelim}
					"type":	"{$type|escape:'html':'UTF-8'}",
					"datas":
					[
					{foreach from=$datas key='index' item='data' name='datas'}
						{ldelim}
						"index":			{$index|escape:'html':'UTF-8'},
...
{if $discounts}{foreach from=$discounts item=discount name='discounts'}
	{ldelim}
		"id":              "{$discount.id_discount|escape:'html':'UTF-8'}",
		...
		"code":            "{$discount.code|escape:'html':'UTF-8'}",
		"link":            "{$link->getPageLink("$order_process", true, NULL, "deleteDiscount={$discount.id_discount|escape:'html':'UTF-8'}")|escape:'html'}",
		...
		"price_float":     "{if $priceDisplay == 1}{$discount.value_tax_exc|escape:'html':'UTF-8'}{else}{$discount.value_real|escape:'html':'UTF-8'}{/if}"
	{rdelim}
...
"nbTotalProducts": "{$nb_total_products|escape:'html':'UTF-8'}",
