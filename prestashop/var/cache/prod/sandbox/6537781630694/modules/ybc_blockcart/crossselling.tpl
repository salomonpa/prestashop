...
{if isset($orderProducts) && count($orderProducts) > 0}
	...
	<div id="blockcart_list">
		<ul {if count($orderProducts) > 4}style="width: {math equation="width * nbImages" width=58 nbImages=$orderProducts|@count}px"{/if}>
			...
			<li>
				<a href="{$orderProduct.link|escape:'html':'UTF-8'}" title="{$orderProduct.name|htmlspecialchars}" class="lnk_img"><img src="{$orderProduct.image|escape:'html':'UTF-8'}" alt="{$orderProduct.name|htmlspecialchars}" /></a>
				<p class="product_name"><a href="{$orderProduct.link|escape:'html':'UTF-8'}" title="{$orderProduct.name|htmlspecialchars}">{$orderProduct.name|truncate:15:'...'|escape:'html':'UTF-8'}</a></p>
				...
				{else}
					<br />
				{/if}
				<!-- <a title="{l s='View' mod='blockcart'}" href="{$orderProduct.link|escape:'html':'UTF-8'}" class="button_small">{l s='View' mod='blockcart'}</a><br /> -->
			</li>
			{/foreach}
		</ul>
	</div>
