...
{block name="field"}
	...
	{if $input.type == 'file' && (!isset($input.imageType) || isset($input.imageType) && $input.imageType!='thumb')&&  isset($display_img) && $display_img}
        ...
        <div class="col-lg-9">
    		<a  class="ybc_fancy" href="{$display_img}"><img title="{l s='Click to see full size image'}" style="display: inline-block; max-width: 200px;" src="{$display_img}" /></a>
            {if isset($img_del_link) && $img_del_link && !(isset($input.required) && $input.required)}
                <a onclick="return confirm('{l s='Do you want to delete this image?'}');" style="display: inline-block; text-decoration: none!important;" href="{$img_del_link}"><span style="color: #666"><i style="font-size: 20px;" class="process-icon-delete"></i></span></a>
            {/if}
        </div>
    {elseif $input.type == 'file' && isset($input.imageType) && $input.imageType=='thumb' &&  isset($display_thumb) && $display_thumb}
	    <label class="control-label col-lg-3" style="font-style: italic;">{l s='Uploaded image: '}</label>
        <div class="col-lg-9">
    		<a  class="ybc_fancy" href="{$display_thumb}"><img title="{l s='Click to see full size image'}" style="display: inline-block; max-width: 200px;" src="{$display_thumb}" /></a>
            {if isset($thumb_del_link) && $thumb_del_link && !(isset($input.required) && $input.required)}
                <a onclick="return confirm('{l s='Do you want to delete this image?'}');" style="display: inline-block; text-decoration: none!important;" href="{$thumb_del_link}"><span style="color: #666"><i style="font-size: 20px;" class="process-icon-delete"></i></span></a>
            {/if}
        </div>
    {/if}
{/block}
	...
	{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
		<div class="panel-footer">
            {if isset($cancel_url) && $cancel_url}
                <a class="btn btn-default" href="{$cancel_url}"><i class="process-icon-cancel"></i>{l s='Cancel'}</a>
            {/if}
